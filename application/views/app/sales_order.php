							...
							<div class="listItem item transition "  data-category="transition">
								<div class="in ">
									<div>
										...
										<div class="text-small text-secondary "><label class="text-bold">Cust. PO. No. : </label> <?=$row->doc_no?> | <label class="text-bold">Del. Date : </label> <?= formatDate($row->cod_date)?></div>
										<div class="text-small  text-secondary \"><label class="text-bold">SO. No. : </label> <?=(getPrefixNumber($row->trans_prefix,$row->trans_no))?> | <label class="text-bold">SO. Date : </label><?=date("d-m-Y", strtotime($row->trans_date))?></div>
										<div class="text-small  text-secondary \"><label class="text-bold">Wo. No. : </label> <?=$row->wo_no?></div>

									</div>
									<div class="text-end">
										<?php
										$printBtn = '<a class="dropdown-item leadAction"  href="'.base_url("salesOrder/salesOrder_pdf/".$row->trans_main_id).'" >
