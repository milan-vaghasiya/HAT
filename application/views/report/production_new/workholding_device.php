                            ...
                            <table id='reportTable' class="table table-bordered">
								<thead class="thead-info" id="theadData">
                                    <tr class="text-center">
                                        <th colspan="5"><?=$pageHeader?></th>
                                        <th colspan="2">D/PRD/03 (00/01.01.16)</th>
                                    </tr>
									<tr class="text-center">
										<th>#</th>
								...
								<tbody>
									...
									foreach($workHoldingData as $row):
										$size = (!empty($row->size)) ? $row->size : $row->rev_specification ;
										echo '<tr>
											<td>'.$i++.'</td>
											<td>'.$row->item_name.'</td>
...
$(document).ready(function(){
    ...
    $(document).on('click','.loadPdf',function(e){
		...
		if(valid)
		{
        	window.open(base_url + controller + '/printWorkHoldingDevice').focus();
        }
    });
