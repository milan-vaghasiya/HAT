...
class AutoCron extends CI_Controller{
    ...
    public function updateShiftlogV1(){
		...
		//$day=1; $cmonth=3; $cyear=2023; $inserted=0; $updated=0;
		$day = date('d');$cmonth = date('m');$cyear = date('Y');$inserted=0;$updated=0;
        if(!empty($empData)):
			foreach($empData as $row):
				...
				for($fkey=1;$fkey<=intVal(date('t',strtotime(date($cyear.'-'.$cmonth.'-01'))));$fkey++)
				{
