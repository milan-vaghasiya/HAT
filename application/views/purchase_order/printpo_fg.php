	...
	<div class="col-12">
		...
		<table class="table item-list-bb" style="margin-top:10px;">
			<tr>
				...
				<th style="width:75px;">Our Final Machining Size</th>
			...
			<?php
				...
				if(!empty($poData->itemData)):
					foreach($poData->itemData as $row):
						...
						$finishDime = (!empty($row->make_brand))?explode(' X ',$row->make_brand):"";
						$dimention = '<b>ID:-</b> '.(!empty($finishDime[0])?$finishDime[0]:'').'<br>';
						$dimention .= '<b>OD:-</b> '.(!empty($finishDime[1])?$finishDime[1]:'').'<br>';
						$dimention .= '<b>Length:-</b> '.(!empty($finishDime[2])?$finishDime[2]:'');
