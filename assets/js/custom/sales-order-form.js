$(document).ready(function(){
	...
	$(document).on("change keyup","#party_id",function(){
		...
		if($(this).val() != ""){
			...
			var gstin = partyData.gstin;
			$('#trans_mode').val(partyData.contact_person);
			var stateCode = "";
			if(gstin != ""){
				stateCode = gstin.substr(0, 2);
...
// 			}else {
				...
				if(((formData.qty == "" || formData.qty == "0") && (formData.qty_kg == "" || formData.qty_kg == "0")) || (formData.price == "" || formData.price == "0" || formData.price == "0.000")){
					...
					if(formData.disc_per == "" && formData.disc_per == "0"){
						amount = parseFloat(parseFloat(formData.qty) * (parseFloat(formData.price) + parseFloat(formData.packing_charge))).toFixed(2);
					}else{
						total = parseFloat(parseFloat(formData.qty) * (parseFloat(formData.price) + parseFloat(formData.packing_charge))).toFixed(2);
	...
	$(document).on('click','.add-item',function(){
		...
		$('#material_grade option').removeAttr('disabled');
		$("#grn_data").removeAttr('readonly');
	});
...
function AddRow(data) {
	...
	var mgradeInput = $("<input/>",{type:"hidden",name:"material_grade[]",value:data.material_grade});
	...
	var targetDateInput = $("<input/>",{type:"hidden",name:"target_date[]",value:data.target_date});
	...
	var packingChInput = $("<input/>",{type:"hidden",name:"packing_charge[]",value:data.packing_charge});
	cell = $(row.insertCell(-1));
	cell.html(data.packing_charge);	
	cell.append(packingChInput);
	...
	var btnEditItem = $('<button><i class="ti-pencil-alt"></i></button>');
    btnEditItem.attr("type", "button");
    btnEditItem.attr("onclick", "EditItem("+JSON.stringify(data)+",this);");
    btnEditItem.attr("class", "btn btn-outline-warning waves-effect waves-light");

	if(data.production_qty != '' && data.production_qty != 0){
		cell.append(btnEditItem);
	}else{
		cell.append(btnEdit);
...
function EditItem(data,button){
	...
	var row_index = $(button).closest("tr").index();
	$("#itemModel").modal();
	var item_id = ""; var material_grade = "";
	...
	$("#item_id option").attr('disabled',true);
	if(item_id){
		$('#item_id option[value='+item_id+']').removeAttr('disabled');
	}
	$("#material_grade option").attr('disabled',true);
	if(material_grade){
		$('#material_grade option[value='+material_grade+']').removeAttr('disabled');
	}
	$("#grn_data").attr('readonly','readonly');
