...
// Service Workers
//-----------------------------------------------------------------------
if (AKSHARAPP.PWA.enable) {
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('https://akshar.scubeerp.in/service_worker.js')
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("AKSHARAPPAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("AKSHARAPPAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("AKSHARAPPDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (AKSHARAPP.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (AKSHARAPP.Dark_Mode.local_mode.enable) {
    var nightStart = AKSHARAPP.Dark_Mode.local_mode.start_time;
    var nightEnd = AKSHARAPP.Dark_Mode.local_mode.end_time;
...
if (AKSHARAPP.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("AKSHARAPPDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("AKSHARAPPDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("AKSHARAPPDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%cAKSHARAPP", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(AKSHARAPP.PWA.enable, "PWA")
    testModeMsg(AKSHARAPP.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(AKSHARAPP.Dark_Mode.local_mode.enable, "Local dark mode (between " + AKSHARAPP.Dark_Mode.local_mode.start_time + ":00 and " + AKSHARAPP.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(AKSHARAPP.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(AKSHARAPP.RTL.enable, "RTL")
    testModeMsg(AKSHARAPP.Test.enable, "Test mode")
    testModeMsg(AKSHARAPP.Test.alert, "Test mode alert")
    ...
    testModeMsg(AKSHARAPP.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = AKSHARAPP.Test.word;
    var value = "";
