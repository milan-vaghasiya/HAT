            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-md-3"> <h4 class="card-title">Monthly Attendance</h4></div>
							<div class="col-md-9">
                                <div class="input-group">
									<select name="biomatric_id" id="biomatric_id" class="form-control single-select req" style="width:250px;">
										
										<?php
										    if($this->loginId == -1 OR $this->loginId == 1){ echo '<option value="ALL">ALL</option>'; }
											foreach($empList as $row):
												if(!empty($row->biomatric_id)):
												    $selected='';
												    if($this->loginId != -1 AND $this->loginId != 1){ $selected = ($this->loginId == $row->id)?'selected':'disabled'; }
									...
									<button type="button" class="btn waves-effect waves-light btn-warning float-right" title="Load Data" style="padding: 0.3rem 0px;border-radius:0px;width:12%;" onclick="printMonthlySummary('excel');"><i class="fa fa-file-excel"></i> Excel</button>
									<button type="button" class="btn waves-effect waves-light btn-primary float-right" title="Load Data" style="padding: 0.3rem 0px;width:12%;border-top-left-radius:0px;border-bottom-left-radius:0px;" onclick="printMonthlySummary('pdf');"><i class="fa fa-file-pdf"></i> PDF</a>
									<!--<button type="button" class="btn waves-effect waves-light btn-success float-right" title="Load Data" style="padding: 0.3rem 0px;width:12%;border-top-left-radius:0px;border-bottom-left-radius:0px;" onclick="printMonthlySummary('hourly');"><i class="fa fa-file-pdf"></i> Hourly</a>-->
								</div>
                            </div>                        
                        </div>                                         
                    </div>
