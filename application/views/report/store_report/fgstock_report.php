                            ...
                            <table id='reportTable' class="table table-bordered">
								<thead class="thead-info" id="theadData">
                                    <tr class="text-center">
                                        <th colspan="9">Stock Statement (Finish Product)</th>
                                        <th colspan="2">F/STR/02 (00/01.01.16)</th>
                                    </tr>
									<tr>
										<th>#</th>
										<th>Part No.</th>
...
$(document).ready(function(){
    ...
    $(document).on('click','.loaddata',function(e){
		...
		var sendData = {party_id:party_id, to_date:to_date,type:type};
		if(valid)
		{
			if(type == 1){
				var url =  base_url + '/reports/storeReport/getFgStockReport/' + encodeURIComponent(window.btoa(JSON.stringify(sendData)));
				window.open(url);
				
			}else{
				$.ajax({
					url: base_url + controller + '/getFgStockReport',
					data: {party_id:party_id, to_date:to_date,type:type},
					type: "POST",
					dataType:'json',
