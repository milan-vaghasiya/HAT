$(document).ready(function(){
	...
	$(document).on('click',".addNew",function(){
        ...
        $.ajax({ 
			...
			setTimeout(function(){ initMultiSelect();setPlaceHolder();initModalSelect(); }, 5);
        });
    });	
...
function edit(data){
	...
	$.ajax({ 
		...
		initMultiSelect();setPlaceHolder(); initModalSelect();
	});
}
...
*/
function editWithHtml(data){
	var button = "";
	var button = data.button;if(button == "" || button == null){button="both";}
	var fnEdit = data.fnEdit;if(fnEdit == "" || fnEdit == null){fnEdit="edit";}
	var fnsave = data.fnsave;if(fnsave == "" || fnsave == null){fnsave="save";}
	var fshowHtml = data.showHtml !='' ? data.showHtml : false;
	var fnhtml = data.fnhtml !='' ? data.fnhtml : '';
	...
	$.ajax({ 
		...
		if(fshowHtml && fnhtml !=''){
			showHtml(data.id,fnhtml);
		}
		initMultiSelect();setPlaceHolder(); initModalSelect();
	});
}

function showHtml(inv_id,fnhtml="getPackingSlipItemList"){
	$.ajax({
		url : base_url + controller + '/' + fnhtml,
		type: 'POST',
		data:{'inv_id':inv_id},
		dataType:'json',
		success:function(data){
			$('.itemListing').html(data.htmlData);
		}
	});
}
