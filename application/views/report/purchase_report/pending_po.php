                            ...
                            <table id='reportTable' class="table table-bordered">
								<thead class="thead-info" id="theadData">
                                    <tr class="text-center">
                                        <th colspan="9">PENDING PURCHASE ORDER REPORT</th>
                                    </tr>
									<tr class="text-center">
										<th>#</th>
...
$(document).ready(function(){
    ...
    $(document).on('click','.loaddata',function(e){
		...
		{
			if(type == 1){
				var url =  base_url + '/reports/purchaseReport/getPendingPO/' + encodeURIComponent(window.btoa(JSON.stringify(sendData)));
				window.open(url);
				
			}else{
				
				$.ajax({
					url: base_url + controller + '/getPendingPO',
					data: sendData,
					type: "POST",
					dataType:'json',
